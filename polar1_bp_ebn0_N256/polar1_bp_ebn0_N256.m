...
Gi = zeros(floor(N*R), N);
% encoding matrix for frozen bit
Gf = zeros(floor(N*(1-R)), N);
% source_bits=randi([0,1],1,NN);
source_bits=zeros(1,NN);
for i=1:NN/k
    x(i,:)=source_bits(k*(i-1)+1:k*i);
end
frozen_bits=randi([0,1],1,N-k);
encoded_bits=zeros(1,N);%极化码编码矩阵
received_sample=zeros(1,N);
...
for i_aw=1:length(SNR)
    ...
    for r_idx = 1:length(source_bits)/k
        decoded_info_one=zeros(1,k);
        init_lr=zeros(1,N);
        for j=1:N  
            init_lr(j)=exp(-2*received_sample((r_idx-1)*N+j)*(2*ebn0(i_aw)));
            tmp = init_lr(j);
            if (tmp == 0)
%                 error('value error!');
...
%         end
        ...
        final_lr = zeros(1,N)';
        frozen_bits_one=frozen_bits(k_f*(r_idx-1)+1:k_f*(r_idx-1)+k_f);
...
%            end
    ...
    BER_BP(1,i_aw)=sum(decoded_info ~= source_bits)/NN
    PER_BP(1,i_aw)=sum(gp_BER~=0)/(length(source_bits)/k)
    disp('循环次数')
    i_aw
end
BER_BP
