function [polar_info_esti, CRC_check] = SPC_CASCL_decoder(llr, L, info_index, frozen_bits, G, poly, lambda_offset, llr_layer_vec, bit_layer_vec, mutual_bits)%2018.1.7.14:16 Yu Y. R.
...
%codes.
%const
%Modify the CA-SCL decoder, the estimate bits are not in sequence, but be
%consistent with the info_index.

%Commit -m: "For SPC, the CRC check of the CA-SCL decoder can not be the
%same as that of the NSPC." Date:2019/10/08.
...
u = zeros(N, L);%unfrozen bits that polar codes carry, including crc bits.
...
for l_index = 1 : L
    ...
    info_with_crc = u(info_index, path_num); 
    encode_bits = mod(info_with_crc' * G, 2);   %Trans to encode bits.
    err = sum(crccheck_m(encode_bits, poly));
    if err == 0
        polar_info_esti = u(info_index, path_num);
        CRC_check = 0;
        break;
    else
        if l_index == L
            polar_info_esti = u(info_index, path_ordered(1));
        end
    end
end
end